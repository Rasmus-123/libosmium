#ifndef OSMIUM_IO_DETAIL_PBFE_OUTPUT_FORMAT_HPP
#define OSMIUM_IO_DETAIL_PBFE_OUTPUT_FORMAT_HPP
...
#include <osmium/extended/io/detail/protobuf_extended_tags.hpp>
...
static_assert(false, "libosmium requires changes for pbfe to be supported!");
#endif
    ...
    namespace io {
        ...
        namespace detail {
            ...
            struct pbf_output_options {
                 ...
                 * PBF blobs?
                 */
                osmium::io::detail::pbf_compression use_compression = osmium::io::detail::pbf_compression::zlib;
            ...
            class DenseNodes {
                ...
                osmium::io::detail::StringTable* m_stringtable;
            ...
            public:
                ...
                DenseNodes(osmium::io::detail::StringTable* stringtable, const pbf_output_options* options) :
            ...
            public:
                ...
                explicit PrimitiveBlock(const pbf_output_options& options, OSMFormat::PrimitiveGroup type, size_t bucket_count) :
                    ...
                    m_stringtable(osmium::io::detail::StringTable::default_stringtable_chunk_size, bucket_count),
                    m_options(options),
                    m_type(type) {
                }
                 ...
                 */
                enum {
                    max_used_blob_size = osmium::io::detail::max_uncompressed_blob_size * 95U / 100U
                };
            ...
            class SerializeBlob {
                ...
                osmium::io::detail::pbf_compression m_use_compression;
            ...
            public:
                 ...
                 */
                SerializeBlob(std::string&& msg, pbf_blob_type type, osmium::io::detail::pbf_compression use_compression, int compression_level) :
                    m_msg(std::move(msg)),
                 ...
                 * @param compression_level Compression level.
                 */
                SerializeBlob(std::shared_ptr<PrimitiveBlock> block, pbf_blob_type type, osmium::io::detail::pbf_compression use_compression, int compression_level) :
                ...
                std::string operator()() {
                    ...
                    assert(m_msg.size() <= osmium::io::detail::max_uncompressed_blob_size);

                    std::string blob_data;
                    protozero::pbf_builder<FileFormat::Blob> pbf_blob{blob_data};

                    switch (m_use_compression) {
                        case osmium::io::detail::pbf_compression::none:
                            pbf_blob.add_bytes(FileFormat::Blob::optional_bytes_raw, m_msg);
                            break;
                        case osmium::io::detail::pbf_compression::zlib:
                            ...
                            break;
                        case osmium::io::detail::pbf_compression::lz4:
#ifdef OSMIUM_WITH_LZ4
                            pbf_blob.add_int32(FileFormat::Blob::optional_int32_raw_size, int32_t(m_msg.size()));
            ...
            class PBFOutputFormat : public osmium::io::detail::OutputFormat, public osmium::handler::Handler {
                ...
                std::size_t m_bucket_count = osmium::io::detail::StringTable::min_bucket_count;
            ...
            public:
                ...
                PBFOutputFormat(osmium::thread::Pool& pool, const osmium::io::File& file, osmium::io::detail::future_string_queue_type& output_queue) :
                    ...
                    m_options.use_compression = osmium::io::detail::get_compression_type(file.get("pbf_compression"));
                ...
                void write_header(const osmium::io::Header& header) final {
                    ...
                    if (!header.boxes().empty()) {
                        ...
                        pbf_header_bbox.add_sint64(OSMFormat::HeaderBBox::required_sint64_left,   int64_t(box.bottom_left().lon() * osmium::io::detail::lonlat_resolution));
                        pbf_header_bbox.add_sint64(OSMFormat::HeaderBBox::required_sint64_right,  int64_t(box.top_right().lon()   * osmium::io::detail::lonlat_resolution));
                        pbf_header_bbox.add_sint64(OSMFormat::HeaderBBox::required_sint64_top,    int64_t(box.top_right().lat()   * osmium::io::detail::lonlat_resolution));
                        pbf_header_bbox.add_sint64(OSMFormat::HeaderBBox::required_sint64_bottom, int64_t(box.bottom_left().lat() * osmium::io::detail::lonlat_resolution));
                    }

                    pbf_header_block.add_string(OSMFormat::HeaderBlock::repeated_string_required_features, "OsmSchema-V0.6_AreaExtension-V0.1");
            ...
            const bool registered_pbfe_output = osmium::io::detail::OutputFormatFactory::instance().register_output_format(osmium::io::file_format::pbfe,
                [](osmium::thread::Pool& pool, const osmium::io::File& file, osmium::io::detail::future_string_queue_type& output_queue) {
                    return new PBFOutputFormat{pool, file, output_queue};
            });

            // dummy function to silence the unused variable warning from above
            inline bool get_registered_pbfe_output() noexcept {
                return registered_pbfe_output;
            }
...
#endif // OSMIUM_IO_DETAIL_PBFE_OUTPUT_FORMAT_HPP
