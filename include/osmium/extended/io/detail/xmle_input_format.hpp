...
{
    constexpr bool debug_builder = false;
     ...
     */
    struct xml_error : public osmium::io_error {
     ...
     */
    struct format_version_error : public osmium::io_error {
    ...
    namespace detail {
        ...
        class XMLEParser final : public osmium::io::detail::ParserWithBuffer {
            ...
            class ExpatXMLParser {
                ...
                void member_wrap(XMLEParser& xml_parser, TFunc&& func) noexcept {
                    if (m_exception_ptr) {
                        return;
                    }
                    try {
                ...
                static void wrap(void* data, TFunc&& func) noexcept {
                    assert(data);
                    auto& xml_parser = *static_cast<XMLEParser*>(data);
                ...
                static void XMLCALL start_element_wrapper(void* data, const XML_Char* element, const XML_Char** attrs) noexcept {
                    wrap(data, [&](XMLEParser& xml_parser) {
                ...
                static void entity_declaration_handler(void* data,
                    ...
                    wrap(data, [&](XMLEParser& /*xml_parser*/) {
                        throw osmium::extended::xml_error{ "XML entities are not supported" };
                        });
                }
            ...
            public:
                ...
                void operator()(const std::string& data, bool last) {
                    ...
                    if (XML_Parse(m_parser, data.data(), static_cast<int>(data.size()), last) == XML_STATUS_ERROR) {
                        ...
                        throw osmium::extended::xml_error{ m_parser };
                    }
                }
            ...
            void top_level_element(const XML_Char* element, const XML_Char** attrs) {
                if (!std::strcmp(element, "osm")) {
                    ...
                    throw osmium::extended::xml_error{ std::string{"Unknown top-level element: "} + element };
                }
                ...
                check_attributes(attrs, [this](const XML_Char* name, const XML_Char* value) {
                    if (!std::strcmp(name, "version")) {
                        ...
                        if (std::strcmp(value, "0.6") != 0) {
                            throw osmium::extended::format_version_error{ value };
                        }
                    }
                ...
                if (m_header.get("version").empty()) {
                    throw osmium::extended::format_version_error{};
                }
            }
            ...
            void start_element(const XML_Char* element, const XML_Char** attrs) {
                        ...
                        if (read_types() & osmium::osm_entity_bits::relation) {
                            ...
                            check_attributes(attrs, [&type, &ref, &ref_is_set, &role](const XML_Char* name, const XML_Char* value) {
                                if (!std::strcmp(name, "type")) {
                                    type = osmium::char_to_item_type(value[0]);
                                }
                            ...
                            if (type != osmium::item_type::node && type != osmium::item_type::way && type != osmium::item_type::relation) {
                                throw osmium::extended::xml_error{ "Unknown type on relation <member>" };
                            }
                            if (!ref_is_set) {
                                throw osmium::extended::xml_error{ "Missing ref on relation <member>" };
                            }
                ...
                case context::comment:
                    ...
                    break;
                case context::text:
                    throw osmium::extended::xml_error{ "No element in <text> allowed" };
                case context::bounds:
                    throw osmium::extended::xml_error{ "No element in <bounds> allowed" };
                case context::obj_bbox:
                    throw osmium::extended::xml_error{ "No element in <bbox>/<bounds> allowed" };
                case context::other:
        ...
        public:
            ...
            explicit XMLEParser(osmium::io::detail::parser_arguments & args) :
                ParserWithBuffer(args) {
            }

            XMLEParser(const XMLEParser&) = delete;
            XMLEParser& operator=(const XMLEParser&) = delete;

            XMLEParser(XMLEParser&&) = delete;
            XMLEParser& operator=(XMLEParser&&) = delete;

            ~XMLEParser() noexcept override = default;
        ...
        const bool registered_xmle_parser = osmium::io::detail::ParserFactory::instance().register_parser(
            osmium::io::file_format::xmle,
            [](osmium::io::detail::parser_arguments& args) {
                return std::unique_ptr<osmium::io::detail::Parser>(new XMLEParser{ args });
            });

        // dummy function to silence the unused variable warning from above
        inline bool get_registered_xmle_parser() noexcept {
            return registered_xmle_parser;
        }
